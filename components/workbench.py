...
def render_workbench(db):
    ...
    if unified:
        ...
        tab_unified, tab_geo, tab_paper, tab_creator, tab_history = st.tabs([
            "统一早报", "GEO", "论文", "博主", "历史"
        ])
        ...
        for tab, mid in [(tab_geo, "geo"), (tab_paper, "ai_papers"), (tab_creator, "creators")]:
            with tab:
        ...
        with st.container():
            ...
            with sc2:
                if st.button("抓取论文", use_container_width=True, key="wb_paper"):
                    with st.spinner("抓取 AI 论文..."):
                        from scrapers.rss_scraper import RSSScraper
                        from processors.content_processor import ContentProcessor
                        items = []
                        try:
                            from scrapers.arxiv_scraper import ArxivScraper
                            items = ArxivScraper(module="ai_papers").fetch_all(verbose=False)
                        except ImportError:
                            pass
                        rss = RSSScraper(module="ai_papers")
                        items.extend(rss.fetch_all(verbose=False))
                        proc = ContentProcessor(db, module="ai_papers")
                        stats = proc.process_and_save(items)
                        st.success(f"论文: 新增 {stats['new_saved']} / 总 {stats['total_fetched']}")

            with sc3:
                if st.button("抓取博主/Twitter", use_container_width=True, key="wb_creator"):
                    with st.spinner("抓取博主和 Twitter..."):
                        from processors.content_processor import ContentProcessor
                        items = []
                        try:
                        ...
                        try:
                            ...
                            items.extend(TwitterScraper().fetch_all(verbose=False))
                        except Exception:
                            pass
                        proc = ContentProcessor(db, module="creators")
                        stats = proc.process_and_save(items)
                        st.success(f"博主: 新增 {stats['new_saved']} / 总 {stats['total_fetched']}")
